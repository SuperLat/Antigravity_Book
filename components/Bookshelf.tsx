...
import { Book, Plus, BookOpen, Download, Trash2, Archive, Loader2, Upload } from 'lucide-react';
...
export const Bookshelf: React.FC<BookshelfProps> = ({ books, onSelectBook, onCreateBook, onDeleteBook, onImportBook }) => {
              ...
              <div className={`h-32 bg-gradient-to-br ${book.cover || 'from-gray-700 to-gray-800'} p-4 relative`}>
                <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="p-1.5 bg-black/50 hover:bg-indigo-500/80 rounded-full text-white backdrop-blur-sm transition-colors"
                    title="导出/归档到本地"
                  >
                    {exportingId === book.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Archive className="w-4 h-4" />}
                  </button>
                  <button 
                    onClick={(e) => { e.stopPropagation(); onDeleteBook(book.id); }}
                    className="p-1.5 bg-black/50 hover:bg-red-500/80 rounded-full text-white backdrop-blur-sm transition-colors"
                    title="删除作品"
                  >
