...
export const IdeaLab: React.FC<IdeaLabProps> = ({ 
  ...
  const [stageModels, setStageModels] = useState<{
    ...
    spark: settings.ai.modelName,
    world: settings.ai.modelName,
    plot: settings.ai.modelName,
    beats: settings.ai.modelName
  });

  // Init models with default if not set (or if settings change significantly, we could update, but user override is better to keep)
  useEffect(() => {
    // We only set defaults on mount or if explicit reset needed. 
    // Here we ensure at least something is set.
  ...
  const handleGenerateWorld = async () => {
    ...
    try {
      ...
      const tempConfig = { ...settings.ai, modelName: stageModels.spark };
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      const tempConfig = { ...settings.ai, modelName: stageModels.world };
  ...
  const handleGenerateBeats = async () => {
    ...
    try {
      ...
      const tempConfig = { ...settings.ai, modelName: stageModels.beats };
      ...
      <div className="w-64 border-r border-gray-800 bg-gray-900/50 flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {ideas.map(idea => (
            <div 
              key={idea.id} 
              className={`group flex items-stretch w-full rounded transition-colors border mb-1 overflow-hidden ${
        ...
        <div className="flex-1 flex flex-col h-full bg-gray-950">
          ...
          <div className="flex border-b border-gray-800 px-8">
            <button 
              ...
              className={`py-4 mr-8 text-sm font-medium flex items-center border-b-2 transition-colors ${
                activeStage === 'spark' ? 'border-yellow-500 text-yellow-500' : 'border-transparent text-gray-500 hover:text-gray-300'
              }`}
            >
            ...
            <button 
              ...
              className={`py-4 mr-8 text-sm font-medium flex items-center border-b-2 transition-colors ${
                activeStage === 'world' ? 'border-blue-500 text-blue-500' : 'border-transparent text-gray-500 hover:text-gray-300'
              }`}
            >
            ...
            <button 
              ...
              className={`py-4 mr-8 text-sm font-medium flex items-center border-b-2 transition-colors ${
                activeStage === 'plot' ? 'border-purple-500 text-purple-500' : 'border-transparent text-gray-500 hover:text-gray-300'
              }`}
            >
